...
//#region node_modules/.nitro/vite/services/ssr/assets/artwork-media.service-ClnAtzMv.js
...
async function setPrimaryArtworkMedia(artworkId, mediaId) {
	const { data: media, error } = await supabase.from("artwork_media").select("id").eq("artwork_id", artworkId);
	if (error) throw error;
	if (!media?.some((item) => item.id === mediaId)) throw new Error("Selected photo does not belong to this artwork.");
	const { error: clearError } = await supabase.from("artwork_media").update({ role: "detail" }).eq("artwork_id", artworkId);
	if (clearError) throw clearError;
	const { error: primaryError } = await supabase.from("artwork_media").update({ role: "primary" }).eq("id", mediaId).eq("artwork_id", artworkId);
