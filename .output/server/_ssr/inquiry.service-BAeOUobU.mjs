...
//#region node_modules/.nitro/vite/services/ssr/assets/inquiry.service-BAeOUobU.js
...
async function getInquiryByToken(token) {
	...
	if (error) throw error;
	if (data.link_expires_on && new Date(data.link_expires_on) <= /* @__PURE__ */ new Date()) return null;
	return data;
}
async function saveShippingAddress(inquiryId, address) {
	const now = (/* @__PURE__ */ new Date()).toISOString();
	const { data: current, error: currentError } = await supabase.from("inquiries").select("inquiry_status").eq("id", inquiryId).single();
	if (currentError) throw currentError;
	const { data, error } = await supabase.from("inquiries").update({
		...
		...withStatusProgress(current.inquiry_status, "ADDRESS_RECEIVED", now)
	}).eq("id", inquiryId).select(`
      *,
      artworks (
        id,
...
//#endregion
export { formatDateTime as a, getEffectiveInquiryStatus as c, saveShippingAddress as d, updateArtistNotes as f, formatDateOnly as i, getInquiryById as l, STATUS_LABELS as n, generateCustomerLink as o, updateInquiryStatus as p, createInquiry as r, getAllInquiries as s, STATUS_FLOW as t, getInquiryByToken as u };
