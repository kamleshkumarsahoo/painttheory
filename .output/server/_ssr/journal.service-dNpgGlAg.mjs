...
//#region node_modules/.nitro/vite/services/ssr/assets/journal.service-dNpgGlAg.js
var STORAGE_BUCKET = "artworks";
var JOURNAL_FOLDER = "journal";
...
function mapJournal(row) {
	const media = [...row.journal_media ?? []].sort((a, b) => a.sort_order - b.sort_order).map(mapMedia);
	return {
		id: row.id,
		title: row.title,
		body: row.body,
...
async function createJournal(entry) {
	const { data, error } = await supabase.from("journal").insert({
		...
		published_at: entry.published_at ?? (/* @__PURE__ */ new Date()).toISOString(),
...
async function updateJournal(id, entry) {
	const { data, error } = await supabase.from("journal").update({
		...entry,
...
async function toggleJournalPinned(id, pinned) {
	const { error } = await supabase.from("journal").update({ pinned }).eq("id", id);
	if (error) throw error;
}
async function uploadJournalMedia({ journalId, file, role, sortOrder, altText }) {
	const mediaId = v4();
	const extension = file.name.split(".").pop()?.toLowerCase() || "jpg";
	const basePath = `${JOURNAL_FOLDER}/${journalId}/${mediaId}`;
	...
	try {
		for (const upload of uploads) {
			const { error } = await supabase.storage.from(STORAGE_BUCKET).upload(upload.path, upload.file, {
				upsert: false,
		...
		await supabase.storage.from(STORAGE_BUCKET).remove(uploads.map((item) => item.path));
		throw error;
	}
}
async function deleteJournalMedia(mediaId) {
	const { data, error } = await supabase.from("journal_media").select(`
      id,
      original_path,
      large_path,
      medium_path,
	...
	const { error: deleteError } = await supabase.from("journal_media").delete().eq("id", mediaId);
	if (deleteError) throw deleteError;
}
//#endregion
export { getJournalById as a, uploadJournalMedia as c, getAllJournal as i, deleteJournal as n, toggleJournalPinned as o, deleteJournalMedia as r, updateJournal as s, createJournal as t };
